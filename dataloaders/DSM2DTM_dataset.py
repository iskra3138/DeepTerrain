...
@gin.configurable
class DSM2DTMDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        start_point = 36
        # Padding
        diff_h, diff_w = (self.size+start_point) - B.shape[1], (self.size+start_point) - B.shape[2]
        left = diff_h // 2
        right = diff_h - left
        top = diff_w // 2
        bot = diff_w - top
        ...
        # Cropping & Flipping
        if self.split == 'train' :
            A, B = self.random_crop (A, B, start_point = start_point)
